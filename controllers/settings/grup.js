...
const redisPrefix = process.env.REDIS_PREFIX + "settings:grup:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 7, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_grup`(`ids_level`, `grup`, `deskripsi`, `keterangan`, `status`, `created_by`) VALUES (?, ?, ?, ?, ?, ?)",
            param: [ids_level, grup, deskripsi, keterangan, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_grup: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `views_level_grup`";
        let sqlReadTotalData = "SELECT COUNT(ids_grup) as total FROM `views_level_grup`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('deskripsi', deskripsi, 'LIKE');
...
Controller.update = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_grup FROM `tbs_grup` WHERE ids_grup = ? LIMIT 1',
            param: [id],
        });
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_grup\` SET ${updates.join(', ')} WHERE \`ids_grup\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_grup` WHERE ids_grup = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `views_level_grup`";
        const params = [];
