...
const redisPrefix = process.env.REDIS_PREFIX + "settings:hak_akses:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 8, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_hak_akses FROM `tbs_hak_akses` WHERE ids_level = ? AND ids_modul = ? LIMIT 1',
            param: [ids_level, ids_modul],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_hak_akses`(`ids_level`, `ids_modul`, `permission`, `created_by`) VALUES (?, ?, ?, ?)",
            param: [ids_level, ids_modul, permission, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_hak_akses: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `views_hak_akses`";
        let sqlReadTotalData = "SELECT COUNT(ids_hak_akses) as total FROM `views_hak_akses`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('ids_modul', ids_modul);
        addCondition('modul', modul, 'LIKE');
        addCondition('ids_level', ids_level);
        addCondition('level', level, 'LIKE');
        addCondition('ids_hak_akses', ids_hak_akses);
        addCondition('aksi', aksi, 'LIKE');
        addCondition('permission', permission, 'LIKE');
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_hak_akses FROM `tbs_hak_akses` WHERE ids_hak_akses = ? LIMIT 1',
            param: [id],
        });
        ...
        addUpdate('ids_hak_akses', ids_hak_akses);
        addUpdate('ids_level', ids_level);
        addUpdate('ids_modul', ids_modul);
        addUpdate('permission', permission);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_hak_akses\` SET ${updates.join(', ')} WHERE \`ids_hak_akses\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_hak_akses` WHERE ids_hak_akses = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `views_hak_akses`";
        const params = [];
