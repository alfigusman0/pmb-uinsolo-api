...
const redisPrefix = process.env.REDIS_PREFIX + "settings:daya_tampung:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 6, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_daya_tampung FROM `tbs_daya_tampung` WHERE kode_jurusan = ? AND YEAR(date_created) = ? LIMIT 1',
