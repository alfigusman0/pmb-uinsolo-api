...
const redisPrefix = process.env.REDIS_PREFIX + "settings:level:";
...
Controller.create = async (req, res) => {
    try {
        ...
        // Check permission
        if (tingkat < req.authTingkat) {
            const message = 'You do not have permission to create a level with a higher tier than your own.';
            return response.sc403(message, {}, res);
        }

        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT level FROM `tbs_level` WHERE level = ? LIMIT 1',
            param: [level],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_level`(`level`, `tingkat`, `status`, `created_by`) VALUES (?, ?, ?, ?)",
            param: [level, tingkat, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_level: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `tbs_level`";
        let sqlReadTotalData = "SELECT COUNT(level) as total FROM `tbs_level`";
        const params = [];
        const totalParams = [];
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT level FROM `tbs_level` WHERE ids_level = ? LIMIT 1',
            param: [id],
        });
        ...
        // Check permission
        if (req.authTingkat > checkData[0].tingkat) {
            const message = 'You do not have permission to update a level with a higher tier than your own.';
            return response.sc403(message, {}, res);
        }
        ...
        addUpdate('level', level);
        addUpdate('tingkat', tingkat);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_level\` SET ${updates.join(', ')} WHERE \`ids_level\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_level FROM `tbs_level` WHERE ids_level = ? LIMIT 1',
            param: [id],
        });
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_level` WHERE ids_level = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `tbs_level`";
        const params = [];
