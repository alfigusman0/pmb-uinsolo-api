...
const redisPrefix = process.env.REDIS_PREFIX + "settings:tipe_file:";
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT setting FROM `tbs_tipe_file` WHERE setting = ? AND nama_file = ? LIMIT 1',
            param: [setting, nama_file],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_tipe_file`(`setting`, `nama_file`, `extensi`, `max_size`, `upload`, `template`, `jalur_masuk`, `tipe_ujian`, `status`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            param: [setting, nama_file, extensi, max_size, upload, template, jalur_masuk, tipe_ujian, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_tipe_file: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `tbs_tipe_file`";
        let sqlReadTotalData = "SELECT COUNT(ids_tipe_file) as total FROM `tbs_tipe_file`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('extensi', extensi, 'LIKE');
        addCondition('upload', upload);
        addCondition('template', template, 'LIKE');
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_tipe_file FROM `tbs_tipe_file` WHERE ids_tipe_file = ? LIMIT 1',
            param: [id],
        });
        ...
        addUpdate('nama_file', nama_file);
        addUpdate('extensi', extensi);
        addUpdate('max_size', max_size);
        addUpdate('upload', upload);
        addUpdate('template', template);
        addUpdate('jalur_masuk', jalur_masuk);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_tipe_file\` SET ${updates.join(', ')} WHERE \`ids_tipe_file\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_tipe_file` WHERE ids_tipe_file = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `tbs_tipe_file`";
        const params = [];
