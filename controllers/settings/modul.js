...
const redisPrefix = process.env.REDIS_PREFIX + "settings:modul:";
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT modul FROM `tbs_modul` WHERE modul = ? LIMIT 1',
            param: [modul],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_modul`(`modul`, `aksi`, `status`, `created_by`) VALUES (?, ?, ?, ?)",
            param: [modul, aksi, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_modul: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `tbs_modul`";
        let sqlReadTotalData = "SELECT COUNT(modul) as total FROM `tbs_modul`";
        const params = [];
        const totalParams = [];
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_modul FROM `tbs_modul` WHERE ids_modul = ? LIMIT 1',
            param: [id],
        });
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_modul\` SET ${updates.join(', ')} WHERE \`ids_modul\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_modul` WHERE ids_modul = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `tbs_modul`";
        const params = [];
