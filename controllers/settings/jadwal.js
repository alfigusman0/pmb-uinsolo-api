...
const redisPrefix = process.env.REDIS_PREFIX + "settings:jadwal:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 9, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_jadwal FROM `tbs_jadwal` WHERE ids_tipe_ujian = ? AND tanggal = ? AND ids_ruangan = ? AND jam_awal = ? AND jam_akhir = ? LIMIT 1',
            param: [ids_tipe_ujian, tanggal, ids_ruangan, jam_awal, jam_akhir],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_jadwal`(`ids_tipe_ujian`, `tanggal`, `jam_awal`, `jam_akhir`, `ids_ruangan`, `quota`, `status`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            param: [ids_tipe_ujian, tanggal, jam_awal, jam_akhir, ids_ruangan, quota, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_jadwal: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `views_jadwal`";
        let sqlReadTotalData = "SELECT COUNT(ids_jadwal) as total FROM `views_jadwal`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('ids_jadwal', ids_jadwal);
        ...
        addCondition('tanggal', tanggal);
        addCondition('jam_awal', jam_awal);
        addCondition('jam_akhir', jam_akhir);
        addCondition('ids_area', ids_area);
        addCondition('area', area, 'LIKE');
        addCondition('ids_gedung', ids_gedung);
        addCondition('gedung', gedung, 'LIKE');
        addCondition('ids_ruangan', ids_ruangan);
        addCondition('ruangan', ruangan, 'LIKE');
        addCondition('quota', quota, '>=');
        addCondition('status', status);
        addCondition('YEAR(date_created)', tahun);
        ...
        getData.forEach(item => {
            item.tanggal = helper.convertoDate(item.tanggal);
        });
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_jadwal FROM `tbs_jadwal` WHERE ids_jadwal = ? LIMIT 1',
            param: [id],
        });
        ...
        addUpdate('ids_jadwal', ids_jadwal);
        addUpdate('ids_tipe_ujian', ids_tipe_ujian);
        addUpdate('tanggal', tanggal);
        addUpdate('jam_awal', jam_awal);
        addUpdate('jam_akhir', jam_akhir);
        addUpdate('ids_ruangan', ids_ruangan);
        addUpdate('quota', quota);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_jadwal\` SET ${updates.join(', ')} WHERE \`ids_jadwal\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_jadwal` WHERE ids_jadwal = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `views_jadwal`";
        const params = [];
        ...
        getData[0].tanggal = helper.convertoDate(getData[0].tanggal);
