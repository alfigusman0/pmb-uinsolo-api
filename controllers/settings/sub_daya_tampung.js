...
const redisPrefix = process.env.REDIS_PREFIX + "settings:sub_daya_tampung:";
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_sub_daya_tampung FROM `tbs_sub_daya_tampung` WHERE kode_jurusan = ? AND ids_jalur_masuk = ? AND YEAR(date_created) = ? LIMIT 1',
            param: [kode_jurusan, ids_jalur_masuk, new Date().getFullYear()],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_sub_daya_tampung`(`kode_jurusan`, `ids_jalur_masuk`, `daya_tampung`, `status`, `created_by`) VALUES (?, ?, ?, ?, ?)",
            param: [kode_jurusan, ids_jalur_masuk, daya_tampung, status, req.authIdUser]
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_sub_daya_tampung: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `views_sub_daya_tampung`";
        let sqlReadTotalData = "SELECT COUNT(ids_sub_daya_tampung) as total FROM `views_sub_daya_tampung`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('ids_sub_daya_tampung', ids_sub_daya_tampung);
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_sub_daya_tampung FROM `tbs_sub_daya_tampung` WHERE ids_sub_daya_tampung = ? LIMIT 1',
            param: [id],
        });
        if (!checkData.length) {
            return response.sc404('Data not found.', {}, res);
        ...
        // Check if kode_jurusan already exists for another record
        const checkKodeJurusan = await helper.runSQL({
            sql: 'SELECT ids_sub_daya_tampung FROM `tbs_sub_daya_tampung` WHERE kode_jurusan = ? AND ids_jalur_masuk = ? AND YEAR(date_created) = ? AND ids_sub_daya_tampung != ? LIMIT 1',
            param: [kode_jurusan, ids_jalur_masuk, new Date().getFullYear(), id],
        });
        if (checkKodeJurusan.length) {
            return response.sc400('Kode jurusan already exists for another record.', {}, res);
        }
        ...
        addUpdate('ids_sub_daya_tampung', ids_sub_daya_tampung);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_sub_daya_tampung\` SET ${updates.join(', ')} WHERE \`ids_sub_daya_tampung\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_sub_daya_tampung` WHERE ids_sub_daya_tampung = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `views_sub_daya_tampung`";
        const params = [];
