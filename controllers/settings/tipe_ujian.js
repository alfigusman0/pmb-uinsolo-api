...
const redisPrefix = process.env.REDIS_PREFIX + "settings:tipe_ujian:";
...
Controller.create = async (req, res) => {
    try {
        ...
        const checkData = await helper.runSQL({
            sql: 'SELECT kode FROM `tbs_tipe_ujian` WHERE kode = ? LIMIT 1',
            param: [kode],
        });
        if (checkData.length) {
        ...
        const sqlInsert = {
            sql: "INSERT INTO `tbs_tipe_ujian`(`ids_program`, `tipe_ujian`, `kode`, `status_jadwal`, `quota`, `status`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?)",
            param: [ids_program, tipe_ujian, kode, status_jadwal, quota, status, req.authIdUser],
        };

        const result = await helper.runSQL(sqlInsert);
        const json = {
            ids_tipe_ujian: result.insertId
        };
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `views_tipe_ujian`";
        let sqlReadTotalData = "SELECT COUNT(ids_tipe_ujian) as total FROM `views_tipe_ujian`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('status_jadwal', status_jadwal);
...
Controller.update = async (req, res) => {
    try {
        ...
        // Check existing data
        const checkData = await helper.runSQL({
            sql: 'SELECT ids_tipe_ujian FROM `tbs_tipe_ujian` WHERE ids_tipe_ujian = ? LIMIT 1',
            param: [id],
        });
        ...
        addUpdate('ids_program', ids_program);
        addUpdate('tipe_ujian', tipe_ujian);
        addUpdate('kode', kode);
        addUpdate('status_jadwal', status_jadwal);
        ...
        const sqlUpdate = {
            sql: `UPDATE \`tbs_tipe_ujian\` SET ${updates.join(', ')} WHERE \`ids_tipe_ujian\` = ?`,
            param: [...params, id]
        };
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        const sqlDelete = {
            sql: 'DELETE FROM `tbs_tipe_ujian` WHERE ids_tipe_ujian = ?',
            param: [id],
        };
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `views_tipe_ujian`";
        const params = [];
