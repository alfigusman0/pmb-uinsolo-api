...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:pendidikan:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 56, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_pendidikan` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_pendidikan`(`idp_formulir`, `jenjang`, `nama_univ`, `status_univ`, `fakultas`, `jurusan`, `akreditasi`, `jalur_penyesuaian_studi`, `ipk`, `tgl_lulus`, `gelar`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            param: [idp_formulir, jenjang, nama_univ, status_univ, fakultas, jurusan, akreditasi, jalur_penyesuaian_studi, ipk, tgl_lulus, gelar, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_pendidikan: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `tbp_pendidikan`";
        let sqlReadTotalData = "SELECT COUNT(idp_pendidikan) as total FROM `tbp_pendidikan`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_pendidikan', idp_pendidikan);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_pendidikan FROM `tbp_pendidikan` WHERE idp_pendidikan = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_pendidikan SET ${updates.join(', ')} WHERE idp_pendidikan = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_pendidikan` WHERE idp_pendidikan = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `tbp_pendidikan`";
        const params = [];
