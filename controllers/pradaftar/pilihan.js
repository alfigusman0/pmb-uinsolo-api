...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:pilihan:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 58, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_pilihan` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_pilihan`(`idp_formulir`, `pilihan`, `kode_jurusan`, `created_by`) VALUES (?, ?, ?, ?)',
            param: [idp_formulir, pilihan, kode_jurusan, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_pilihan: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_pilihan`";
        let sqlReadTotalData = "SELECT COUNT(idp_pilihan) as total FROM `viewp_pilihan`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_pilihan', idp_pilihan);
        addCondition('idp_formulir', idp_formulir);
        addCondition('pilihan', pilihan);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_pilihan FROM `tbp_pilihan` WHERE idp_pilihan = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_pilihan SET ${updates.join(', ')} WHERE idp_pilihan = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_pilihan` WHERE idp_pilihan = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_pilihan`";
        const params = [];
