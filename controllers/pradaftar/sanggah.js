...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:sanggah:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 65, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const ids_sanggah = (req.authIdsLevel === 5) ? 1 : req.body.ids_sanggah;
        const {
            idp_formulir,
            sanggah,
        } = req.body;
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_sanggah` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_sanggah`(`idp_formulir`, `sanggah`, `ids_sanggah`, `created_by`) VALUES (?, ?, ?, ?)',
            param: [idp_formulir, sanggah, ids_sanggah, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_sanggah: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_sanggah`";
        let sqlReadTotalData = "SELECT COUNT(idp_sanggah) as total FROM `viewp_sanggah`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_sanggah', idp_sanggah);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_sanggah FROM `tbp_sanggah` WHERE idp_sanggah = ?';
        const param = [id];
        ...
        addUpdate('ids_sanggah', ids_sanggah);
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_sanggah SET ${updates.join(', ')} WHERE idp_sanggah = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_sanggah` WHERE idp_sanggah = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_sanggah`";
        const params = [];
