...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:formulir:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 51, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const result = await helper.runSQL({
            sql: `INSERT INTO tbp_formulir (nomor_peserta, kategori, ids_tipe_ujian, formulir, pembayaran, ket_pembayaran, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            param: [null, kategori, ids_tipe_ujian, 'BELUM', 'BELUM', null, created_by]
        });

        const json = {
            idp_formulir: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_formulir`";
        let sqlReadTotalData = "SELECT COUNT(idp_formulir) as total FROM `viewp_formulir`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('ids_tipe_ujian', no_ids_tipe_ujian, 'NOT IN');
        addCondition('tipe_ujian', tipe_ujian, 'LIKE');
        addCondition('formulir', formulir);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_formulir FROM `tbp_formulir` WHERE idp_formulir = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_formulir SET ${updates.join(', ')} WHERE idp_formulir = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_formulir` WHERE idp_formulir = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_formulir`";
        const params = [];
