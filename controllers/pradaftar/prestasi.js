...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:prestasi:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 59, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_prestasi` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_prestasi`(`idp_formulir`, `prestasi`, `tingkat`, `sertifikat`, `url`, `created_by`) VALUES (?, ?, ?, ?, ?)',
            param: [idp_formulir, prestasi, tingkat, sertifikat, url, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_prestasi: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `tbp_prestasi`";
        let sqlReadTotalData = "SELECT COUNT(idp_prestasi) as total FROM `tbp_prestasi`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_prestasi', idp_prestasi);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_prestasi FROM `tbp_prestasi` WHERE idp_prestasi = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_prestasi SET ${updates.join(', ')} WHERE idp_prestasi = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_prestasi` WHERE idp_prestasi = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `tbp_prestasi`";
        const params = [];
