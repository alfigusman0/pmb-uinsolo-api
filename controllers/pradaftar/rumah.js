...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:rumah:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 57, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_rumah` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_rumah`(`idp_formulir`, `ids_kelurahan`, `dusun`, `rw`, `rt`, `jalan`, `kode_pos`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            param: [idp_formulir, ids_kelurahan, dusun, rw, rt, jalan, kode_pos, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_rumah: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_rumah`";
        let sqlReadTotalData = "SELECT COUNT(idp_rumah) as total FROM `viewp_rumah`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_rumah', idp_rumah);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_rumah FROM `tbp_rumah` WHERE idp_rumah = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_rumah SET ${updates.join(', ')} WHERE idp_rumah = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_rumah` WHERE idp_rumah = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_rumah`";
        const params = [];
