...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:biodata:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 52, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_biodata` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_biodata`(`idp_formulir`, `nik`, `nama`, `jenis_kelamin`, `tempat_lahir`, `tgl_lahir`, `ids_agama`, `kewarganegaraan`, `ibu_kandung`, `ids_keb_khusus`, `kebutuhan_khusus`, `keterangan`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            param: [idp_formulir, nik, nama, jenis_kelamin, tempat_lahir, tgl_lahir, ids_agama, kewarganegaraan, ibu_kandung, ids_keb_khusus, kebutuhan_khusus, keterangan, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_biodata: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_biodata`";
        let sqlReadTotalData = "SELECT COUNT(idp_biodata) as total FROM `viewp_biodata`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_biodata', idp_biodata);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_biodata FROM `tbp_biodata` WHERE idp_biodata = ?';
        const param = [id];
        ...
        addUpdate('ibu_kandung', ibu_kandung);
        addUpdate('ids_keb_khusus', ids_keb_khusus);
        addUpdate('kebutuhan_khusus', kebutuhan_khusus);
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_biodata SET ${updates.join(', ')} WHERE idp_biodata = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_biodata` WHERE idp_biodata = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_biodata`";
        const params = [];
