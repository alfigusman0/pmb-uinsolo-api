...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:file:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 60, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_file` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_file`(`idp_formulir`, `ids_tipe_file`, `file`, `url`, `created_by`) VALUES (?, ?, ?, ?, ?)',
            param: [idp_formulir, ids_tipe_file, file, url, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_file: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_file`";
        let sqlReadTotalData = "SELECT COUNT(idp_file) as total FROM `viewp_file`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_file', idp_file);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_file FROM `tbp_file` WHERE idp_file = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_file SET ${updates.join(', ')} WHERE idp_file = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_file` WHERE idp_file = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_file`";
        const params = [];
