...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:kelulusan:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 64, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_kelulusan` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_kelulusan`(`idp_formulir`, `kode_jurusan`, `total`, `lulus`, `nama_penitip`, `keterangan`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?)',
            param: [idp_formulir, kode_jurusan, total, lulus, nama_penitip, keterangan, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_kelulusan: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_kelulusan`";
        let sqlReadTotalData = "SELECT COUNT(idp_kelulusan) as total FROM `viewp_kelulusan`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_kelulusan', idp_kelulusan);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_kelulusan FROM `tbp_kelulusan` WHERE idp_kelulusan = ?';
        const param = [id];
        ...
        addUpdate('lulus', lulus);
        addUpdate('nama_penitip', nama_penitip);
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_kelulusan SET ${updates.join(', ')} WHERE idp_kelulusan = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_kelulusan` WHERE idp_kelulusan = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_kelulusan`";
        const params = [];
