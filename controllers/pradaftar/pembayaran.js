...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:pembayaran:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 61, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_pembayaran` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_pembayaran`(`idp_formulir`, `ids_bank`, `va`, `id_billing`, `nominal`, `expire_at`, `created_by`) VALUES (?, ?, ?, ?, ?, ?, ?)',
            param: [idp_formulir, ids_bank, va, id_billing, nominal, expire_at, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_pembayaran: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_pembayaran`";
        let sqlReadTotalData = "SELECT COUNT(idp_pembayaran) as total FROM `viewp_pembayaran`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_pembayaran', idp_pembayaran);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_pembayaran FROM `tbp_pembayaran` WHERE idp_pembayaran = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_pembayaran SET ${updates.join(', ')} WHERE idp_pembayaran = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_pembayaran` WHERE idp_pembayaran = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_pembayaran`";
        const params = [];
