...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:nilai:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 63, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_nilai` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_nilai`(`idp_formulir`, `keterangan`, `nilai`,`created_by`) VALUES (?, ?, ?, ?)',
            param: [idp_formulir, keterangan, nilai, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_nilai: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_nilai`";
        let sqlReadTotalData = "SELECT COUNT(idp_nilai) as total FROM `viewp_nilai`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_nilai', idp_nilai);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_nilai FROM `tbp_nilai` WHERE idp_nilai = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_nilai SET ${updates.join(', ')} WHERE idp_nilai = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_nilai` WHERE idp_nilai = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_nilai`";
        const params = [];
