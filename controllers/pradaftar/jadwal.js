...
const redisPrefix = process.env.REDIS_PREFIX + "pradaftar:jadwal:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 62, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        let checkData = await helper.runSQL({
            sql: 'SELECT idp_formulir FROM `tbp_jadwal` WHERE idp_formulir = ? LIMIT 1',
            param: [idp_formulir],
        });
        if (checkData.length) {
        ...
        const result = await helper.runSQL({
            sql: 'INSERT INTO `tbp_jadwal`(`idp_formulir`, `ids_jadwal`, `created_by`) VALUES (?, ?, ?)',
            param: [idp_formulir, ids_jadwal, created_by],
        });

        const json = {
            idp_formulir: idp_formulir,
            idp_jadwal: result.insertId,
        }
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewp_jadwal`";
        let sqlReadTotalData = "SELECT COUNT(idp_jadwal) as total FROM `viewp_jadwal`";
        const params = [];
        const totalParams = [];
        ...
        addCondition('idp_jadwal', idp_jadwal);
...
Controller.update = async (req, res) => {
    try {
        ...
        /* Check existing data */
        let sql = 'SELECT idp_jadwal FROM `tbp_jadwal` WHERE idp_jadwal = ?';
        const param = [id];
        ...
        await helper.runSQL({
            sql: `UPDATE tbp_jadwal SET ${updates.join(', ')} WHERE idp_jadwal = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbp_jadwal` WHERE idp_jadwal = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewp_jadwal`";
        const params = [];
