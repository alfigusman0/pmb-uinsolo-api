...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `viewd_pembayaran`";
        let sqlReadTotalData = "SELECT COUNT(idd_pembayaran) as total FROM `viewd_pembayaran`";
        const params = [];
        const totalParams = [];
        ...
        getData.forEach(item => {
            item.expire_at = helper.convertoDateTime(item.expire_at);
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `viewd_pembayaran`";
        const params = [];
        ...
        getData[0].expire_at = helper.convertoDateTime(getData[0].expire_at);
